...
\begin{document}
    ...
    \section{Introduction}
        ...
        This paper evaluates the implications of mapping GMG to the Cerebras Wafer-Scale Engine (WSE), a radically different architectural model~\cite{Cerebras,Lauterbach}. By integrating nearly one million processing elements (PEs) and tens of gigabytes of on-wafer SRAM onto a single wafer-scale chip, the WSE provides single-cycle access to local memory and 21 petabytes-per-second of memory bandwidth, along with 214 petabits-per-second of fabric bandwidth through a 2D mesh interconnect. 
        ...
        In this paper, we present GLOW (Gmg Levels On Wafer).  We build on the lessons from the successful use of Cerebras for stencil computations to provide a mapping of an algorithm that is still structured yet significantly more challenging.
        
        Our paper presents an iso-capability (same problem size) study of the WSE (44GB SRAM) and a GH200 GPU (96 GB HBM DRAM).  We present a dispassionate performance analysis of the WSE-3, focusing on the relevant benefits and pitfalls associated with massive decoupled thread parallelism and planar network topologies for multi-scale computations.  As compared to a GH200 GPU implementation of the HPGMG benchmark, we achieve significantly improved performance.  At the same time,
    ...
    \section{Background}
        ...
        \subsection{Geometric Multigrid Methods}
            ...
            Multigrid methods are iterative solvers that solve sparse linear systems of equations by forming a hierarchy of grid levels and iterating across them to %eliminate errors efficiently. 
            ...
            In finite-volume discretizations, restriction corresponds to averaging contributions of eight fine-grid cells into one coarse cell, recursively repeated %The algorithm continues this process 
            ...
            After the coarse solution is computed, the V-cycle traverses back up through the hierarchy. Interpolation transfers coarse-grid correction to the finer grid %using  \[I^2h = Interpolate(x^4h)    >> TODO: Fixme\] Where in interpolate 
            such that a coarse cell is replicated to 8 fine cells, the inverse of restriction. An error-correction step updates the fine-grid solution. % using  \[x^2h = x^2h + I^2h        >>  TODO: Fix me\]
            Post-smoothing reduces any high-frequency errors reintroduced during interpolation. This down-and-up traversal is called a V-cycle, and achieves rapid convergence.
            ...
            \begin{figure}[ht]
                ...
                Figure shows the hardware machinery inside a PE: router, memory and  Compute Element (CE). The WSE-3 system is a giant 46,255 mm² chip along with the machinery needed to cool, power, and provide I/O.} %\textcolor{blue}{Increased font please check if readable}}
                \label{fig:CS3}
            \end{figure}
        ...
        \subsection{Cerebras WSE-3 and Programming Model}
            ...
            The wafer consists of  893{,}064 identical, simple Processing Elements (PEs) arranged in a homogeneous 1172\(\times\)762 2-dimensional grid. Each PE is self-contained with its own Memory, Compute Element (CE), and Router; i.e.,  the WSE-3 is a spatial, distributed-memory, and dataflow-oriented architecture. %PEs don't share data unless communicated explicitly through the fabric. 
            ...
            \subsubsection{Router}
                ...
                Multicasting replicates the wavelet and sends it in all directions with zero cost overhead. The router also has Queues for input and output buffering.
...
\end{comment}
        ...
        \subsection{Mapping Multigrid Operators and Levels} 
        \label{sec:mapping_alternatives}
            Mapping different multigrid levels onto a spatial accelerator with thousands of independent PEs introduces an opportunity for spatial placement of operators and levels across the wafer. %{\color{blue} Along with domain (data) sharding, placement and routing amongst levels.} 
            We considered a number of alternative designs, some placing different operators in different areas of the wafer, and others spatially mapping the levels of the V-cycle.
            ...
            deep learning operators for ML training~\cite{cerebras_execution_modes,cerebras_pipeline_dl}. GMG, however, is ill-suited to a pipelined layout as the levels have irregular data sizes, dependences across levels, and multiple iteration loops.
...
\paragraph{\textit{Spatial Mapping of Levels}} Suppose that different levels of the multigrid computation utilize adjacent portions of the wafer. 
            One such approach has been studied for mapping neural networks on WSE~\cite{cerebras_execution_modes,cerebras_pipeline_dl}.  %{\color{blue} This provides low per-PE memory usage and high stencil locality since neighbors are 1 hop apart. MARY: These things haven't been defined yet -- rewrote with more words.
            ...
            \paragraph{\textit{Distributed Mapping with Spatial Level Mapping}}
            The distributed mapping approach inspired by MPI domain decomposition superimposes layers on top of each other, such that each PE can perform all the operations.
            The finest grid level occupies the full region of the wafer.  A spatial level mapping reorganizes data from coarser levels in contiguous tiles, such that are either mapped towards one side or towards the center of the wafer. The stencils at each level can be highly parallelized, given their nearest neighbor halo exchanges.
...
\begin{figure}[t]
                    ...
                    \caption{Strided distributed mapping of GMG levels, as described in this paper.  %fferent possible layouts for 3 level GMG layers on WSE3 of size 1172x762. Text in red highlights various challenges of each approach. Our approach falls into the distributed category. }
                    }
                ...
                \begin{figure}[t]
                    ...
                    \caption{Top: Stencil operator on WSE-3 2D grid for \(8^3\) problem with 3 levels. Highlighted nodes are active PEs; remaining are inactive PEs. Hop distance grows as one goes down the levels and exchanges halo regions H hops apart. }
                    \label{fig:Spmv_and_restriction}
                \end{figure}
                
            
            \paragraph{\textit{Strided Distributed Mapping (this paper)}}
            Multi-grids typically have more compute and data at the finer grids, so solving them fast is important. Also, as compared to the previous paragraph, avoiding the costly irregular data reorganization at coarser grid levels also allows us to maintain uniform communication across the wafer. 
            The strided distributed mapping (Figure~\ref{fig:VariousLayouts})
            increases the distance at coarser grid levels by powers of 2; i.e., a PE \(2^l\) hops apart is the grid point for that level \(l\). This simpler implementation avoids the bulk data movement between levels required by other distributed mappings, at the cost of increasing the halo exchange distance at coarser levels.
            We describe in detail the strided distributed mapping used in this experiment in the remaining subsections.

        \subsection{Mapping Data from 3D Problem Domain to 2D Wafer} \label{sec:data_layout}
            The GMG input domain is a structured 3D \(nx \times ny \times nz\) grid as shown in Figure~\ref{fig 1:V-cycle in GMG}. We map a 3D problem onto a 2D grid by storing the entire z-dimension (\(nz\) elements) on each PE for a fixed value of \(x\) and \(y\); i.e., a \textit{pencil}. Concretely, PE at position \((i,j)\) on the wafer holds the pencil for grid coordinates \(x=i, y=j\), mapping an \(N^3\) problem size to an \(N \times N\) tile of PEs. %, so the x-axis of the problem maps to PE columns, the y-axis to PE rows, and the z-axis is stored entirely in local memory. For a problem of size \(N^3\), we use an \(N \times N\) region of PEs with \(nz = N\).  
            %Given the 48KB memory on each PE, this decomposition enables us to scale the problem by using more PEs. 
            All data accesses along the \(z\) dimension are local and do not require communication, as %. Such mapping has been studied 
            in previous stencil mappings on WSE~\cite{Rocki,Sai,sai2,mathias}. Given the multi-grid dimensions halve at each level, we have to maintain in memory multiple \textit{pencils} from different levels as we superimpose levels. The total amount of data that fits is a function of the number of levels, \(L\), and the pencil size \(nz_l\) for each level \(l \in L\). Along the z-axis per PE, the allocation forms a finite geometric series with \(|L|\) levels, where \(nz\) denotes the size of the finest-level pencil \(l_0\):
            \[
            \text{Allocation} = nz + \frac{nz}{2} + \frac{nz}{4} + \frac{nz}{8} + \cdots
            = nz \left( 2 - \frac{1}{2^{|L|-1}} \right)
            \]
            \textit{Allocation} represents the amount of data that must fit in 48KB of memory along with other intermediate vectors and kernel buffers, and the program code for the PE.
            ...
            Since \(nz = N\) for an \(N^3\) problem, scaling from \(N^3\) to \((2N)^3\) both quadruples the number of PEs (from \(N^2\) to \(4N^2\)) and doubles the pencil size per PE (from \(N\) to \(2N\)). The maximum problem size is therefore constrained by the 48KB per-PE memory. %: the pencil and its multi-level copies must fit within 48KB alongside the program code.

        \subsection{Implementation of V-Cycle Operators}
...
Table~\ref{tab:code_breakdown} summarizes the components of the GMG implementation. 
The code includes a host-device runtime orchestrator (in Python) and our own module implementations (in CSL) of residual, restriction, and interpolation.  These are integrated with an existing 7-point stencil module for smoothing, and an allreduce communication module for inter-layer transition operators.  %Each module has its own routing and task ID requirements, and all assignments must be partitioned without collision across the limited 24 available hardware colors. 
The pre-existing modules assume 2-dimensional nearest-neighbor communication, which we extended to support multi-level strided communication, Dirichlet boundary conditions, and the forwarding optimization  (Section~\ref{sec:forwarding}). %Table~\ref{tab:code_breakdown} summarizes the implementation.} %along with layout module to map these operators on wafer PEs.
            

                

            \paragraph{Layout and coloring}
The programming model limits to 24 the number of unique IDs representing tasks and communication channels.
                %The layout description partitions 9 colors and 7 task IDs across the three modules.  
The stencil module uses eight colors
                as four bidirectional channels (for N, S, E, and W and their reverse). %C0/C1 for EAST/WEST halo exchange, C2/C3 for the reverse, and C4/C5, C6/C7 for NORTH/SOUTH. 
Even- and odd-indexed PEs swap send/receive assignments in a checkerboard pattern to avoid link
conflicts. The allreduce module uses one additional color. %(C8) with dynamic teardown to reuse it across 4 pipeline stages. 
                %\textcolor{red}{I'm confused here.  What are the 7 task IDs?  16=9+7, but are task IDs colors?}
The V-cycle state machine uses 1 task ID, and 7 more task IDs are used for the three modules, totaling 17 hardware resources. %The layout iterates over 
For every PE, the implementation generates
                position-dependent boundary flags and routing directives for the strided active PE pattern. %Our V-cycle state machine uses 1 additional task ID, totaling 16 hardware resources and various operators as localized functions. 
                %Our layout iterates over every PE, generating position-dependent parameters specific to the multigrid mapping (boundary flags, per-PE routing directives for the strided active PE pattern) and binds them to the kernels/operators.

            \paragraph{Routing and multi-level adaptation}
                The base SDK configures static routing at compile time. %via \texttt{@set\_local\_color\_config} directives that wire input/output queues to the router ports. 
                The strided mapping, however, requires \textit{runtime} route reconfiguration: at level $l$, only PEs at stride $2^l$ are active, and the stencil must exchange halos $2^l$ hops apart rather than with immediate neighbors. %We extended both libraries with \texttt{setHopBasedParams}: at each level transition, every PE recomputes whether it is active, an interior forwarder, or a boundary node, and reconfigures its routing accordingly. 
                For restriction, 
                %added \texttt{setWindowedReductionParams} to the 
                allreduce is modified so that only a $2^l \times 2^l$ window of active PEs participates in each reduction.  Forwarding  (Section~\ref{sec:forwarding}) further configures inactive PEs to relay wavelets directly through router queues, bypassing the memory ramp.

            \paragraph{State machine and callback}
                The V-cycle kernel %(1,103 lines) 
                executes entirely on-wafer as a %28-state 
                fine-grained state machine (Table~\ref{tab:state_machine}). The down-cycle advances through states that set up each level, apply pre-smoothing via the strided stencil, 
                %(\texttt{SMOOTH\_APPLY} $\to$ \texttt{SMOOTH\_UPDATE}, 
                %looping for the configured number of iterations. 
                compute the residual, %(\texttt{RESIDUAL\_APPLY} $\to$ \texttt{RESIDUAL\_COMPUTE})
                and restrict to the next coarser level. %(\texttt{RESTRICT\_REDUCE} $\to$ \texttt{RESTRICT\_DIV}). 
                At the coarsest $2\times2$ grid, a Jacobi bottom solver iterates. The up-cycle reverses: at each level it expands the z-dimension,
                %(\texttt{EXPAND\_Z}), 
                broadcasts the correction from the top-left PE of each window, %(\texttt{BCAST}), 
                applies error correction, %(\texttt{INTERP\_ADD}), 
                and post-smooths. 
                %All transitions are callback-driven: 
                When an asynchronous computation %stencil or allreduce 
                completes, %the library invokes \texttt{f\_callback}, which 
                A callback advances the state variable and reactivates the state machine task, facilitating an implicit barrier in the programming model. %This design executes an 
                An entire V-cycle executes without %returning control to 
                the host; the host intervenes only between complete V-cycle iterations to read the convergence residual. %$\rho$.

\begin{comment}
THIS SECTION IS REDUNDANT WITH 3B
            \paragraph{Data storage within 48KB}
...
\end{comment}
            ...
            \begin{table}[t]
                ...
                \begin{tabular}{|l||r|r|} \hline
                    ...
                    \quad stencil\_3d\_7pts (strided)   & 1,090          & ${\sim}$306 (mod) \\ \hline
                    \quad allreduce (strided)           & 1,522          & ${\sim}$416 (mod) \\ \hline \hline
        ...
        \subsection{Performance Models of Key Operators}
            \label{sec:perfmodel}
            This subsection describes the internals of the key GMG operators in GLOW, adapted to a distributed, tiled architecture. We discuss compute and communication-heavy operators (7-point stencil and restriction) that dominate the V-cycle cost. We use an analytical model to characterize the costs of operators and how they affect runtime.

            \paragraph{Strided 7-point Stencil}
            % We pose our work on prior work in 
            Mathias et al. present  %. They highlight 
            a low-latency 25-point stencil operator with localized broadcast patterns on WSE-2, achieving near peak performance~\cite{mathias}; the authors contributed a   similar 7-point stencil kernel as part of upstream \texttt{sdk-examples}.  Using this code as a starting point, we extend the communication across PEs to fit the strided mapping strategy.
            ...
            must still participate in the communication of data. The stride distance between active PEs changes with level and grows by \(2^l\) in both \(x\) and \(y\) directions.
            The accumulator receives the neighboring halo data from 4 directions (E/W/N/S).   Also, the Dirichlet boundary conditions on edges initialize the halos with the negation of edges. Each active PE holds the stencil coefficients in SRAM and computes the local Laplacian operation across both \(x\) and \(y\) dimensions and the local \(z\) dimension. %, only if the PE is active. 
            For communication, each PE sends data sequentially to all 4 cardinal directions and asynchronously receives. %We observe that this mapping contributes to various costs like contention in network, distance of communication, amount of data over the network, cost of moving data to or from ramp and router.
            ...
            estimates the number of cycles \(T_l\) at level \(l\) for a given problem size. The model has two terms. In the first term, \(C\) is the largest number of wavelets a PE sends or receives, which is a measure of contention. The second term captures the routing cost: \(E\) is total number of hops the network needs to route wavelets; \(N\) is the number of links being used (a PE has 5 bidirectional links); and, \(L\) is the largest number of hops a wavelet has to travel.  Thus, the first term reflects whether contention or routing the communication pattern is dominant. In the second term, \(T_R\) is the router-processor latency (multiplied by 2 for sending/receiving data and an extra cycle for storing the received element), and \(D\) is the longest pipeline sequence of PEs that perform operations depending on previous calculations.  Together, the two terms %provides a 
            model for communication, computation, and memory access costs.

            \begin{equation}\label{eq:comm_model}
                T_l = \max \left( C_l, E_l / N_l + L_l \right) + (2 T_R + 1) D_l
            \end{equation}

            Next, we apply this model for each level of a \(16^3\) sized problem for three levels, as shown in Figure~\ref{fig:Spmv_and_restriction}, with a varying number of hops at each level. The data that must be communicated is the pencil resident at each PE and \(C_l = C_{l_0}/2^l\), where \(C_{l_0}=nz\) and the number of hops %it must be communicated 
            ...
            \begin{equation}
                T_l = \max \left( nz/2^l, C_l*L_l / N_l + L_l \right) + (2 T_R + 1) D_l
                \label{eq:comm_model_applied}
            \end{equation}

            Therefore, %for the context of this paper, 
            the data that must be communicated is the pencil resident at each PE for a given level, and number of hops it must be communicated is a function of the level.  %For our \textit{strided mapping}, 
            Distance between active PEs at level \(l\) is \(D_l = 2^l\), and the pencil size is \(C_l = nz/2^l\).  Then, \(L_l = 2^l\) hops and \(N_l = 4\) directions (one neighbor per direction). Table~\ref{tab:model_communication_costs} instantiates these terms for a \(512^3\) problem across 9 levels.


            \begin{table}[t]
                \centering
                \caption{Communication model (Eq.~\ref{eq:comm_model}) for strided mapping: $C_l{=}\texttt{zDim}_0/2^l$, $L_l{=}D_l{=}2^l$, $N_l{=}4$; $\texttt{zDim}_0{=}512$, $T_R{=}2$. Latency${=}(2T_R{+}1)D_l{=}5D_l$. With forwarding optimization: $D_l{=}2$, Latency${=}5D_l{=}10$ (constant).}
                ...
                \scriptsize
                \begin{tabular}{@{}cr rrrr rr rr@{}}
                    \toprule
                    & & & & & & \multicolumn{2}{c}{\textbf{Strided}} & \multicolumn{2}{c}{\textbf{Forwarding}} \\
                    \cmidrule(lr){7-8} \cmidrule(lr){9-10}
                    $l$ & \textbf{Grid} & $C_l$ & $L_l{=}D_l$ & \textbf{Comm.} & \textbf{Lat.} & \textbf{Total} & & \textbf{Lat.} & \textbf{Total} \\
                    \midrule
                    0 & $512^2$ & 512 &   1 & 512 &    5 &  517 & &  10 &  522 \\
                    1 & $256^2$ & 256 &   2 & 256 &   10 &  266 & &  10 &  266 \\
                    2 & $128^2$ & 128 &   4 & 128 &   20 &  148 & &  10 &  138 \\
                    3 &  $64^2$ &  64 &   8 &  64 &   40 &  104 & &  10 &   74 \\
                    4 &  $32^2$ &  32 &  16 &  32 &   80 &  112 & &  10 &   42 \\
                    5 &  $16^2$ &  16 &  32 &  36 &  160 &  196 & &  10 &   46 \\
                    6 &   $8^2$ &   8 &  64 &  68 &  320 &  388 & &  10 &   78 \\
                    7 &   $4^2$ &   4 & 128 & 132 &  640 &  772 & &  10 &  142 \\
                    8 &   $2^2$ &   2 & 256 & 260 & 1280 & 1540 & &  10 &  270 \\
                    \bottomrule
                \end{tabular}
            \end{table}

            \paragraph{Window-based Restriction}
            For the restriction operator, we adopt a window-based approach at each level.  We adapt the
            allreduce primitive from the upstream \texttt{sdk-examples}, extending it to perform the reduction on hierarchical windows of PEs, of equal height and width, which is a function of the level.
            %The reduction across points to coarsen the grid is performed within a window of equal height and width, which is a function of the level. 
            Let W be the window size at level l, then \(W = 2^{(l+1)}\). The window reduction pattern is shown in Figure~\ref{fig:Spmv_and_restriction}, where the dark green PE on top left is the receiver. Within each window, the algorithm performs a row-wise reduction followed by a column-wise reduction across the window width and height. Due to the geometry of the layout, each window contains exactly two active PEs per direction.
        ...
        \subsection{Forwarding Optimization} \label{sec:forwarding}
            ...
            \end{equation}
            dominates the communication term from level 5 (\(D_l\)) in Table~\ref{tab:comm_model_strided_512}; it equals the number of pipeline stages (hops) between active processing elements (PEs). This growth occurs because each hop requires access to the ramp and local memory, which can transfer only one wavelet at a time. Wavelets traveling across multiple hops must repeatedly pass through memory, causing serialization in the pipeline and increasing latency.
            ...
            % Table 4 merged into Table 3 above
...
\end{comment}
        ...
        \subsection{Experimental Configuration}
            We used the CS-3 cluster at the ALCF~\cite{alcf_aitestbed} (4 WSE-3 machines). One WSE-3 wafer integrates 893,064 PEs in a $1172 \times 762$ 2D mesh at 875~MHz with 44~GB on-chip SRAM. We used Cerebras SDK~\cite{cerebras_sdk} v1.4.0 on a single WSE-3 attached to a host running Linux 4.18.0 on an AMD EPYC 9354P (32 cores, 188~GB RAM).

            % OLD: The GPU baseline runs on an NVIDIA GH200~\cite{tacc_vista} (96~GB HBM3, no Tensor Cores). It derives from HPGMG-FV~\cite{hpgmg_org} (V1: CPU+MPI, fp64), its CUDA port~\cite{hpgmg_cuda} (V2), and a fork adding distributed portability and periodic BCs (V3). Our version extends V3 with Dirichlet BCs and FP32 support, as WSE-3 lacks fp64. Compilation uses \texttt{-O2 -fopenmp} (host) and \texttt{-O2} via NVCC for \texttt{sm\_90}, with build flags \texttt{USE\_REG}, \texttt{USE\_TEX}, \texttt{CUDA\_UM\_ZERO\_COPY}, and \texttt{HOST\_LEVEL\_SIZE\_\allowbreak{}THRESHOLD=0}; block-copy tiles are $32{\times}4{\times}8$, boundary tiles $64{\times}16{\times}16$. Modules loaded: \texttt{gcc/14.2.0}, \texttt{cuda/12.8(g)}, \texttt{openmpi/5.0.5}, \texttt{ucc/1.7.0}, \texttt{ucx/1.20.0}, \texttt{nvpl/26.1}, \texttt{cmake/4.1.1}, \texttt{python3/3.11.8}.
            \begin{sloppypar}
            The GPU baseline runs on an NVIDIA GH200~\cite{tacc_vista} (96~GB HBM3, no Tensor Cores). It derives from HPGMG-FV~\cite{hpgmg_org} (V1: CPU+MPI, fp64), its CUDA port~\cite{hpgmg_cuda} (V2), and a fork adding distributed portability and periodic BCs (V3). Our version extends V3 with Dirichlet BCs and FP32 support, as WSE-3 lacks FP64. Compilation uses the following build flags\footnote{Build flags: \texttt{sm\_90}, \texttt{-O2 -fopenmp} (host), \texttt{-O2}, \texttt{USE\_REG}, \texttt{USE\_TEX}, \texttt{CUDA\_UM\_ZERO\_COPY}, \texttt{HOST\_LEVEL\_SIZE\_THRESHOLD=0}.}; block-copy tiles are $32{\times}4{\times}8$, boundary tiles $64{\times}16{\times}16$, with the following modules loaded:\footnote{\texttt{gcc/14.2.0}, \texttt{cuda/12.8(g)}, \texttt{openmpi/5.0.5}, \texttt{ucc/1.7.0}, \texttt{ucx/1.20.0}, \texttt{nvpl/26.1}, \texttt{cmake/4.1.1}, \texttt{python3/3.11.8}.}
            \end{sloppypar}
       
            Both platforms perform warmup V-cycles and reset timers before timed runs. WSE\nobreakdash-3 timing uses 48-bit hardware counters at 875~MHz ($t(\mu s){=}(cycles/0.875){\times}10^{-3}$); GH200 uses CUDA events. Both capture device-side execution only, excluding data transfers. We report average time per V-cycle and time to solution; solver parameters (grid size, levels, iterations, tolerance, smoothing counts) are identical across platforms.

        \subsection{Problem Description}
            All experiments solve the constant-coefficient 3D Poisson equation on a structured Cartesian grid with uniform spacing $h^2$, a 7-point stencil, and homogeneous Dirichlet BCs. The solver runs entirely on device in FP32. The RHS is $b{=}\sin(\pi x)\sin(\pi y)\sin(\pi z)$; the initial guess is $x{=}0.0$; stencil coefficients are $\alpha{=}{-}6.0/h^{2}$, $\beta{=}1.0/h^{2}$. Jacobi relaxation ($\omega{=}2/3$) is the smoother. The coarsest level is a $2{\times}2$ grid except in shallow experiments. Iterations terminate when the $\ell_\infty$ residual norm falls below $\epsilon{=}10^{-2}$ or after 100 V-cycles. This tolerance reflects the theoretical FP32 accuracy limit for the 3D Poisson operator: $\kappa(A)\cdot\epsilon_{\text{mach}} \approx (C/h^{2})\cdot 1.2{\times}10^{-7} \approx 10^{-2}$ at $512^3$ resolution. Unless otherwise stated, results use a $512^3$ problem with the 6/6/6 (pre/post/coarse) configuration.

\begin{table*}[t]
    \caption{Performance comparison on $512^3$ problem size. TTS = time to solution (s), 1-cycle = single V-cycle time (s), \#Cycles = V-cycles required for inf-norm to reach $10^{-2}$. Shallow refers to a reduced-depth multigrid hierarchy.}
    \label{tab:platform_comparison}
    \centering
    \footnotesize
    \setlength{\tabcolsep}{3pt}
    \begin{tabular}{|l||r|r|r||r|r|r||r|r|r||r|r|r||r|r|r|} \hline
        & \multicolumn{3}{c||}{\textbf{(4/4/6)}}
        & \multicolumn{3}{c||}{\textbf{(4/4/100)}}
        & \multicolumn{3}{c||}{\textbf{(6/6/6)}}
        & \multicolumn{3}{c||}{\textbf{Shallow (6/6/6)}}
        & \multicolumn{3}{c|}{\textbf{(6/6/100)}} \\ \hline
        \textbf{Platform} & \textbf{TTS} & \textbf{\#Cyc} & \textbf{1-cyc}
                          & \textbf{TTS} & \textbf{\#Cyc} & \textbf{1-cyc}
                          & \textbf{TTS} & \textbf{\#Cyc} & \textbf{1-cyc}
                          & \textbf{TTS} & \textbf{\#Cyc} & \textbf{1-cyc}
                          & \textbf{TTS} & \textbf{\#Cyc} & \textbf{1-cyc} \\ \hline \hline
        GH200   & 0.025986 & 2 & 0.012993
                & 0.029886 & 2 & 0.014943
                & 0.034722 & 2 & 0.017361
                & 0.034722 & 2 & 0.017361
                & 0.040568 & 2 & 0.020284 \\ \hline
        WSE-3   & 0.004221 & 6 & 0.000704
                & 0.013376 & 6 & 0.002229
                & 0.003612 & 4 & 0.000903
                & 0.003506 & 5 & 0.000701
                & 0.009716 & 4 & 0.002429 \\ \hline \hline
        Speedup & 6.2$\times$ & -- & 18.5$\times$
                & 2.2$\times$ & -- &  6.7$\times$
                & 9.6$\times$ & -- & 19.2$\times$
                & 9.9$\times$ & -- & 24.8$\times$
                & 4.2$\times$ & -- &  8.4$\times$ \\ \hline
    \end{tabular}
\end{table*}

        \subsection{V-Cycle Variation Performance Comparison}
            % OLD P1: Figure~\ref{fig:hpgmg-speedup} reports the performance of GLOW on WSE-3 compared against HPGMG on NVIDIA GH200. Bars above the speedup of 1 baseline bar mean WSE-3 is faster.
            Figure~\ref{fig:hpgmg-speedup} compares the single V-cycle speedup of GLOW on WSE-3 against HPGMG on NVIDIA GH200 across five multigrid configurations. GLOW consistently outperforms the GH200 at every problem size, with speedups increasing as the grid scales. The best single V-cycle speedup of approximately $25\times$ is observed at the $512{\times}512$ grid for the $6/6/6$ (Shallow) configuration, while configurations with heavy coarse-level smoothing such as $4/4/100$ yield the lowest speedups.

            Table~\ref{tab:platform_comparison} supplements the figure with time to solution (TTS) and number of V-cycles required for convergence at $512^3$. The fastest GLOW configuration ($6/6/6$ Shallow) completes a single V-cycle (including convergence time) in $0.000701$\,s versus $0.017361$\,s on the GH200, a $24.8\times$ per-cycle speedup. Across all five configurations, GLOW delivers $6.7$--$24.8\times$ single V-cycle speedup and $2.2$--$9.9\times$ TTS speedup. The number of V-cycles required for WSE-3 to reach the convergence tolerance is slightly higher than on the GPU, which can be attributed to differences in floating-point arithmetic and solver implementation.
       

            \begin{figure}[h!]
                \centering
                \includegraphics[width=0.95\linewidth]{figures/hpgmg_speedup_barplot.pdf}
                % OLD caption: Speedup comparison between Cerebras CS-3 and NVIDIA GH200 using HPGMG/GLOW across various problem sizes. The bar plot shows relative performance (higher is better). The \% increase indicated on top of 6/6/6(Shallow)(purple) is the relative speedup against the 6/6/6(red). {\color{blue} Timings reflect a single V-cycle only measuring pure operator time, excluding convergence checking cost, on both GPU and WSE.} \textcolor{red}{As Sam was saying in our Monday meeting, you need to report convergence checking.  I can't find anywhere in the paper where you do this.}
                \caption{Single V-cycle speedup of GLOW (WSE-3) over HPGMG (GH200) across problem sizes and multigrid configurations. The $x$-axis denotes the 2D PE grid size under weak scaling. Each group of bars compares five configurations parameterized by pre/post/coarse smoothing iterations. The \% annotations on the $6/6/6$ (Shallow) bars indicate the per-cycle improvement relative to $6/6/6$ (Deep) on WSE-3. Timings reflect a single V-cycle along with convergence cost on both platforms.}
                \label{fig:hpgmg-speedup}
            \end{figure}

            % OLD P2: These performance trends highlight the sensitivity of GMG performance on WSE-3 to V-cycle structure. Increasing the number of coarse-level iterations (*/*/6 vs. */*/100) along with depth of the V-cycle expands communication distances for both inter-level transfers and intra-level stencil operations. In contrast, our implementation handles finer grids well by scaling the problem over the wafer and exploiting parallelism and stencil locality, with shorter communication distances for halo exchanges and fewer hops to travel for inter-layer communication. A \(6/6/6 (Shallow)\) V-cycle with 7 levels outperforms a \(6/6/6 (Deep)\) V-cycle with 9 levels by almost 45\% on the largest problem size, with the cost of increased iterations to reach convergence.
            % OLD P3: As a second observation, speedups grow as problem size scales. GLOW is well suited to finer levels and larger grids, while single-node GPU performance worsens on larger problems due to increased data movement through the memory system.
            % OLD P4: The configurations with a heavy number of coarse solver iterations (i.e., the blue and orange bars) exhibit a U-shaped trend in Figure~\ref{fig:hpgmg-speedup}, as the cost of the coarse solver dominates for larger problems, pushing the performance to lower levels. Configurations with lighter coarse solver iterations (green and red bars) show a linear increase in performance as the problem scales since very less time is spent on the coarse level. A higher bump on the 512 problem size could also point to how baseline GPUs start struggling with larger grids and not GLOW performing better by 2\(\times\) over previous level.
            These trends highlight the sensitivity of GMG performance on WSE-3 to V-cycle structure. Increasing coarse-level iterations ($*/*/6$ vs.\ $*/*/100$) and V-cycle depth expands communication distances for both inter-level transfers and intra-level stencil operations. In contrast, finer levels exploit stencil locality and short-hop halo exchanges effectively. The $6/6/6$ (Shallow) V-cycle with 7 levels outperforms the $6/6/6$ (Deep) V-cycle with 9 levels by up to $45\%$ per V-cycle on the largest problem size; the percentage annotations on the purple bars in Figure~\ref{fig:hpgmg-speedup} quantify this improvement at each grid size.

            Speedups also grow with problem size. GLOW scales the problem across the mapped wafer, keeping communication distances short at fine levels, while GPU performance degrades as data movement through the memory hierarchy becomes the bottleneck at larger grid sizes. Configurations with lighter coarse solvers ($4/4/6$ and $6/6/6$) exhibit monotonically increasing speedup, since fine-level operators, where GLOW excels, dominate V-cycle time. Configurations with heavy coarse-solver iterations ($4/4/100$ and $6/6/100$) exhibit a U-shaped speedup curve: at small problem sizes, GLOW's fine-level advantage yields moderate speedup; at intermediate sizes, the coarse solver running on few PEs with long-distance communication consumes a growing fraction of V-cycle time and reduces overall speedup; at the largest sizes, the GPU's memory-bandwidth becomes severe enough that GLOW regains its advantage despite the coarse-level overhead.
        ...
        \label{sec:impact-of-optimizations-and-memory-constraints}
            \begin{figure}[t]
                ...
                \caption{
                    Per-operator time per level for one V-cycle on a \(512^3\) problem (GLOW on CS-3). Down and up phases are summed at each level. Config: \(6/6/6\) pre/post/coarse, 9 levels, FP32, 7-point stencil. breaks down time per operator for one V-cycle (excluding convergence), summed over down and up phases at each level
                }
                \label{fig:per-operation-timing}
            \end{figure}

            {\textbf{Smooth and Residual} operators
            %old: separate paragraphs for smooth (3 paras), residual (1 sentence), restriction (1 para)
            shown in Figure~\ref{fig:per-operation-timing}
            use the 7-point strided stencil (stride $2^l$ at level $l$) and share a similar performance trend. Cost initially decreases toward coarser levels as active PEs and local data halve per level. Unlike conventional weak scaling, where cost plateaus or improves monotonically, in GLOW, we observe a gradual increase beyond a certain level due to growing hop distance.

            Figure~\ref{fig:spmv-internal} benchmarks the internal costs in detail: Compute time calculates the Laplacian across the x-y and z plane, it halves per level before flattening from fixed overheads. Communication measures the time to send and receive halo regions in all 4 directions. It starts below the compute on fine grids but grows as halo messages traverse longer paths. The crossover point reflects the tradeoff between shrinking data volume and increasing distance. Table~\ref{tab:model_communication_costs} confirms this analytically for both strided and forwarding-optimized variants. Residual behaves similarly, except that the differences in absolute time come from the absence of multiple smoothing iterations.

            \textbf{Restriction} uses a window-based reduction (Figure~\ref{fig:Spmv_and_restriction}) that gathers contributions from $W{-}1$ PEs; its cost grows with window size due to producer-consumer dependences and ramp cost. We apply forwarding optimization to keep this cost low.
            }

            \begin{figure*}[t]
                \centering
                \includegraphics[width=\textwidth]{figures/spmv_internal.png}
                \caption{Stencil micro-benchmark on CS-3: compute vs.\ communication time per level for the 7-point strided stencil used in smooth and residual operators.}
                \label{fig:spmv-internal}
            \end{figure*}


            \textbf{Coarse solver} applies weighted Jacobi relaxation on the coarsest grid. Its cost is very high compared to fine-level operators and increases with the number of coarse iterations, significantly affecting overall performance in deep V-cycles and W-cycles (Figure~\ref{fig:hpgmg-speedup}).

            {\textbf{Interpolation}
            %old: 12 lines repeating multicast insight twice and walking through figure phases
            shows the opposite trend: more expensive on fine grids, progressively cheaper toward coarser levels (Figure~\ref{fig:interpolation-internal}). Cost is dominated by data transfer, not local computation. Interpolation benefits from the hardware’s zero-cost multicast: data is broadcast in all four directions simultaneously, allowing row-wise and column-wise transfers to occur in parallel. It also lacks producer-consumer dependence chains, so PEs forward immediately without ramp access. As data volume shrinks at coarser levels, the multicast advantage outweighs longer distances. In Figure~\ref{fig:interpolation-internal}, we see that at each level the window-leader PE (top-left, as shown in Figure~\ref{fig:Spmv_and_restriction}) expands the z dimension, resets the routes/colors, broadcasts data, and eventually, when data reaches the destination, calculates the error correction locally using tightly coupled DSD operations. Notice the cost to reset routes is mostly constant.
            }

            \begin{figure*}[t]
                \centering
                \includegraphics[width=\linewidth]{figures/interpolation_internal.png}
                \caption{Interpolation micro-benchmark on CS-3: time breakdown per level showing expand-z, route reset, broadcast, and error correction phases.}
        ...
        \subsection{Memory Constraints: Code vs.\ Data Tradeoff}
            Table~\ref{tab:memory_utilization}(Left-top) quantifies per-PE memory usage across all problem sizes. The code section occupies a fixed ${\sim}$23KB (47\%) of the 48KB SRAM and almost remains constant across problem sizes, this is driven by aggressive function inlining performed by the backend LLVM compiler, as well as the inclusion of stencil and collectives library kernels. The data section, which stores solution vectors, residuals, intermediate buffers, and multi-level save arrays, grows from 2.9\% at $4^3$ to 50.7\% at $512^3$, where the PE reaches 98\% capacity. This fixed code footprint, combined with geometrically growing per-level data, imposes a practical scalability limit: the $512^3$ problem is the largest that fits. Wafer utilization ranges from 0.002\% ($4^3$) to 29.4\% ($512^3$), showing that even our largest problem uses less than a third of the 893,064 available PEs.
            
% \begin{table*}[t]
%     \caption{%
%         \textbf{(Left)} Per-PE memory breakdown and wafer utilization across problem sizes (48KB SRAM per PE, 893,064 total PEs).
%         \textbf{(Right)} Per-PE operation breakdown for a $512^3$ V-cycle. FMAC counts 2 FLOPs; FMOV variants contribute 0.
%         Mem(B)${}=(loads{+}stores){\times}4$. Fab(B): fabric traffic from halo exchange; only FMOV32 touches the fabric.
%         Mem AI = Total FLOPs / Mem(B); Fab AI = Total FLOPs / Fab(B). Arithmetic intensity is identical across levels.%
%     }
...
%             \textbf{Size} & \textbf{Code} & \textbf{Data} & \textbf{Tot.} & \textbf{PE\%} \\ \hline \hline
...
%             & \multicolumn{4}{c||}{\textbf{Level 0 ($512^2$ PEs)}} & \multicolumn{4}{c|}{\textbf{Level 7 ($4^2$ PEs)}} \\ \hline
%             \textbf{Op} & \textbf{Cnt} & \textbf{FLOPs} & \textbf{Mem(B)} & \textbf{Fab(B)} & \textbf{Cnt} & \textbf{FLOPs} & \textbf{Mem(B)} & \textbf{Fab(B)} \\ \hline \hline
%             FSUB       &   79,872 &    79,872 &    958,464 &       0 &      624 &      624 &      7,488 &       0 \\ \hline
%             FMAC       &  632,832 & 1,265,664 & 10,125,312 &       0 &    4,944 &    9,888 &     79,104 &       0 \\ \hline
%             FMUL       &    3,072 &     3,072 &     36,864 &       0 &       24 &       24 &        288 &       0 \\ \hline
%             FADD       &   21,504 &    21,504 &    258,048 &       0 &      168 &      168 &      2,016 &       0 \\ \hline
%             FNEG       &  159,744 &   159,744 &  1,277,952 &       0 &    1,248 &    1,248 &      9,984 &       0 \\ \hline
%             FMOV\_MEM  &   36,352 &         0 &    290,816 &       0 &      240 &        0 &      1,920 &       0 \\ \hline
%             FMOV\_ZERO &   92,160 &         0 &    368,640 &       0 &      768 &        0 &      3,072 &       0 \\ \hline
%             FMOV32     &  172,033 &         0 &    688,132 & 688,132 &    1,344 &        0 &      5,376 &   5,376 \\ \hline \hline
%             \textbf{Total} & & \textbf{1,529,856} & \textbf{14,004,228} & \textbf{688,132} & & \textbf{11,952} & \textbf{109,248} & \textbf{5,376} \\ \hline \hline
%             \multicolumn{3}{|l||}{\textbf{Mem AI}: 0.109} & \multicolumn{2}{l||}{\textbf{Fab AI}: 2.223} & \multicolumn{2}{l|}{\textbf{Mem AI}: 0.109} & \multicolumn{2}{l|}{\textbf{Fab AI}: 2.223} \\ \hline
%         \end{tabular}
%     \end{minipage}
% \end{table*}
...
\begin{table*}[t]
    \caption{%
        \textbf{(Left-Top)} Per-PE memory breakdown and wafer utilization across problem sizes (48KB SRAM per PE, 893,064 total PEs).
        \textbf{(Left-Bottom)} V-cycle vs.\ W-cycle on $256^3$, 8 levels, 6/6/6 (pre/post/coarse) smoothing, $\gamma=2$. Both converge in 10 iterations.
        \textbf{(Right)} Per-PE operation breakdown for a $512^3$ V-cycle. FMAC counts 2 FLOPs; FMOV variants contribute 0.
        Mem(B)${}=(loads{+}stores){\times}4$. Fab(B): fabric traffic from halo exchange; only FMOV32 touches the fabric.
        Mem AI = Total FLOPs / Mem(B); Fab AI = Total FLOPs / Fab(B). Arithmetic intensity is identical across levels.}
        %\color{red}{Left tables should be pulled up, right can move a little towards right margin, unable to fix this need help}
        %
...
\begin{tabular}{|l||r|r|r|} \hline
    \textbf{Metric} & \textbf{V-cyc} & \textbf{W-cyc} & \textbf{Ratio} \\
 \hline \hline \hline
    Iterations& 3& 2&0.7x \\ \hline
    Avg cycle ($\mu$s)      &    544.9&  15,181.9& 27.9x\\ \hline
    Time to sol.\ ($\mu$s)  &  1634.8& 30,487.2& 18.6x\\ \hline
    Conv.\ rate (dec/iter)  &     1.16&      2.62&  2.3x\\ \hline
    Final $\rho_{\infty}$ & 5.53e-03 & 2.21e-03 & \\
\hline
\end{tabular}
    \end{minipage}
%    \hfill%
...
\begin{tabular}{|l||r|r|r|r||r|r|r|r|} \hline
            & \multicolumn{4}{c||}{\textbf{Level 0 ($512^2$ PEs)}} & \multicolumn{4}{c|}{\textbf{Level 7 ($4^2$ PEs)}} \\ \hline
            \textbf{Op} & \textbf{Cnt} & \textbf{FLOPs} & \textbf{Mem(B)} & \textbf{Fab(B)} & \textbf{Cnt} & \textbf{FLOPs} & \textbf{Mem(B)} & \textbf{Fab(B)} \\ \hline \hline
            FSUB       &   26,624 &    26,624 &    319,488 &       0 &      208 &      208 &      2,496 &       0 \\ \hline
            FMAC       &  210,944 &   421,888 &  3,375,104 &       0 &    1,648 &    3,296 &     26,368 &       0 \\ \hline
            FMUL       &    1,024 &     1,024 &     12,288 &       0 &        8 &        8 &         96 &       0 \\ \hline
            FADD       &    7,168 &     7,168 &     86,016 &       0 &       56 &       56 &        672 &       0 \\ \hline
            FNEG       &   53,248 &    53,248 &    425,984 &       0 &      416 &      416 &      3,328 &       0 \\ \hline
            FMOV\_MEM  &   11,776 &         0 &     94,208 &       0 &       80 &        0 &        640 &       0 \\ \hline
            FMOV\_ZERO &   30,720 &         0 &    122,880 &       0 &      256 &        0 &      1,024 &       0 \\ \hline
            FMOV32     &   57,345 &         0 &    229,380 & 229,380 &      448 &        0 &      1,792 &   1,792 \\ \hline \hline
            \textbf{Total} & & \textbf{509,952} & \textbf{4,665,348} & \textbf{229,380} & & \textbf{3,984} & \textbf{36,416} & \textbf{1,792} \\ \hline \hline
            \multicolumn{3}{|l||}{\textbf{Mem AI}: 0.1093} & \multicolumn{2}{l||}{\textbf{Fab AI}: 2.2232} & \multicolumn{2}{l|}{\textbf{Mem AI}: 0.1094} & \multicolumn{2}{l|}{\textbf{Fab AI}: 2.2232} \\ \hline \hline
            \multicolumn{9}{|p{0.66\textwidth}|}{\scriptsize\textit{Per-op definitions (FLOPs; mem loads,stores):}
            FMAC\,=\,2; 3L,1S.\;
            FSUB, FMUL, FADD\,=\,1; 2L,1S.\;
            FNEG\,=\,1; 1L,1S.\;
            FMOV\_MEM\,=\,0; 1L,1S.\;
            FMOV\_ZERO\,=\,0; 0L,1S.\;
            FMOV32\,=\,0; 0L,1S\,+\,1 fab load.\;
            All FP32 (4\,B). Mem(B)${}=(L{+}S){\times}4$; Fab(B)${}=\text{FMOV32}\,{\times}\,4$.} \\ \hline
        \end{tabular}
    \end{minipage}
\end{tabular}
\end{table*}
        ...
        \subsection{On V, W and F cycles}
            We chose to investigate only the V-cycle in this paper because it is by far the most common multigrid schedule in practice, and our per-level performance analysis lets the reader translate findings to F-, W-, or $\mu$-cycle variants. From a computational-efficiency standpoint, all parallel architectures favor the V-cycle: it hides coarse-grid latency behind the throughput of fine grids. On the WSE, this effect is more pronounced because coarse-grid operations pay a communication cost that grows with level: the active-PE stride and communication distance doubles per level, %the hop distance grows with it, 
            and the low-radix network makes each operation at coarser levels %$\ell$ 
            increasingly latency-dominated.
            
            From experiments in Table~\ref{tab:vcycle_vs_wcycle}(Left-bottom), the W-cycle is $31.3\times$ slower per 1 cycle and $27.8\times$ slower to solution as it repeatedly pays coarse-level communication penalty. This confirms that deeper cycle schedules are poorly matched to the strided mapping on WSE, also supplemented by our shallow cycle experiments.

        }

        {
            \subsection{On Higher-Order Stencils}
        %    As such, it acts as a necessary first step when analyzing emerging arch
        %   -itectures. Sophisticated stencils like 7-point variable coefficient stencils are common in real wor
        %   -ld applications and 27-point stencils can provide 4th order accuracy, however they might end up wit
        %   -h significantly more memory and complicated inter-PE communication patterns. This work uses 7-point
        %   - constant coefficient stencil which is ubiquitous in computer science performance optimization comm
        %   -unity. As such, it acts as a necessary first step when analyzing emerging architectures. Given the
        %   -current state of CSL tooling and library support, a 27-point stencil implementation on WSE is itsel
        %   -f a significant contribution as previously highlighted in ~\cite{mathias}

            We use the 7-point constant-coefficient stencil as a standard first step for analyzing emerging architectures. Variable-coefficient and 27-point stencils offer higher accuracy but require significantly more memory and complex inter-PE communication. Given the current state of CSL tooling and library support, a 27-point WSE implementation is by itself a notable contribution as previously highlighted by~\cite{mathias}.
        }
        
        
        \subsection{Roofline Analysis}
        
            The roofline model~\cite{roofline} is used to understand an algorithm's performance. Our roofline is based on the per-operation FLOP and traffic counting methodology followed by Sai et~al.~\cite{sai_fv_sc24} and Jacquelin et~al.~\cite{mathias}. We count the occurrences of each DSD (Data Structure Descriptor) vector operation executed during one V-cycle for PE(0,0), which occurs in all levels. Table~\ref{tab:roofline_ops}~(Right) shows these counters for the finest (L0, $512^2$~PEs) and a coarse level (L7, $4^2$~PEs). Each operation type contributes a known number of FLOPs and memory/fabric accesses as defined in the legend of Table~\ref{tab:roofline_ops}~(Bottom-right). As an example, at Level~0 each PE executes 210,944~FMACs: $2 \times 210{,}944 = 421{,}888$~FLOPs and $210{,}944 \times 4\text{(3L+1S)} \times 4\text{B} = 3{,}375{,}104$~B memory traffic, plus 57,345~FMOV32s contributing $57{,}345 \times 4 = 229{,}380$~B fabric traffic. Together with other operations this totals 509,952~FLOPs, yielding $\text{AI}_{\text{mem}} {=} 0.109$ and $\text{AI}_{\text{fab}} {=} 2.22$~FLOP/Byte. FMAC dominates FLOPs at both levels, driven by the stencil multiply-accumulate in smooth and residual operators.

            Each PE executes one FMAC per cycle (2~FLOPs at 875~MHz), yielding a per-PE peak of 1.75~GFLOP/s on WSE-3. Memory bandwidth is 28~GB/s per PE; fabric bandwidth is 14.0~GB/s per PE (4~directions~$\times$~4~B~$\times$~875~MHz). The corresponding ridge points are $\text{AI}_{\text{mem}}^{*} {=} 0.0625$ and $\text{AI}_{\text{fab}}^{*} {=} 0.125$~FLOP/Byte. At the system level, the per-level peak scales with active PEs: L0 uses 262,144~PEs (458.8~TFLOP/s), halving at each level down to 4~PEs at L8 (7.0~GFLOP/s), as shown in Figure~\ref{fig:roofline}~(Right). Full-wafer peak (893,064~PEs) is 1.56~PFLOP/s. For comparison, on WSE-2~\cite{sai_fv_sc24,mathias} the system peaks are 1.785~PFLOP/s compute, 20.0~PB/s memory, and 3.3~PB/s fabric; WSE-3 provides comparable compute (1.56~PFLOP/s) with higher memory bandwidth (25.0~PB/s) and nearly $4\times$ higher fabric bandwidth (12.5~PB/s).
 
            \begin{figure*}[t]
                \centering
                \includegraphics[width=\textwidth]{figures/roofline_plot.png}
                
                \caption{Roofline analysis for GLOW on a $512^3$ problem with 9 multigrid levels for 6/6/6 config. \textbf{Left:} per-PE~(0,0) performance vs.\ arithmetic intensity; memory and fabric rooflines are shown as separate ceilings.. \textbf{Right:} system-level performance where per-level peak scales with active PEs at each level of multigrid.}
                \label{fig:roofline}
            \end{figure*}
            

            Figure~\ref{fig:roofline} reveals four key observations.
            First, GLOW operates in the \emph{compute-bound} region at fine levels, whereas multigrid on GPUs is typically \emph{memory-bound}. The WSE's flat SRAM hierarchy eliminates cache misses that which dominate GPU stencil performance. Second, AI is nearly identical across all levels as both FLOPs and traffic scale proportionally in GMG. The measured memory AI of 0.109 exceeds the memory ridge point (0.0625), and the fabric AI of 2.22 far exceeds the fabric ridge point (0.125), confirming that neither memory nor fabric bandwidth limits performance.  Third, despite being compute-bound, fine level achieves 43\% of per-PE peak. We attribute this gap to: (a)~control overhead in the callback-driven state machine (state machine dispatches, DSD configurations, Route setup and teardown). Lastly, performance degrades sharply at coarse levels: L5 to L8 reach only ~4\% to 0.1\% of peak, with absolute work per PE drops by $128\times$ from L0 to L8, while growing hop distance and router latency explain the efficiency collapse. 
        
    \section{Lessons Learned}
    This implementation of GMG on WSE-3 required a substantial restructuring effort, and we share some of the lessons from this process.  

    %\paragraph{Multi-scale algorithms on spatial architectures}
 Our approach to mapping a multi-scale algorithm on a spatial architecture balanced    
several competing concerns: memory footprint per PE, communication distance between levels, packing/repacking cost, stencil locality, chip utilization, and routing/coloring complexity. None of the alternative mapping strategies dominates across all of these dimensions. 
The strided distributed mapping we chose prioritizes stencil parallelism and regular communication patterns; it avoids bulk inter-level data movement with irregular communication patterns.  This regularity came at increased communication costs at coarser levels and under-utilization of the wafer.  

There are several ways to increase wafer utilization with the potential to either improve performance or increase the problem size supported. Since our chief bottleneck was limited memory size on the WSE-3, we might benefit from a 
hybrid mapping strategy that decouples computation and storage, such as assigning subsets of PEs primarily as memory resources while others focus on computation.  We could also have employed more dynamic coloring and routing schemes to reduce code replication and improve memory utilization across levels.  

        
%        \paragraph{Further improvements to forwarding optimization.}  
Reducing forwarding cost in the strided communication might benefit from techniques to change the routing, e.g., using control wavelets and runtime coloring reconfiguration~\cite{mathias}.
...
%        6. Poeple should not have to go to 27pt because of what oscar explained me, write it. They choose 27 pt because say 7pt was memory bound and using 27 pt the performance at least moves up, now given that GLOW is alredy compute bound we can't get speedups using 27pt as we are already there, one benefit is of accuracy using 27pt. there can be other implications, say 27 will be less power hungry.
    ...
    Historically, operations on large structured grids led to extensive work on blocking and tiling optimizations for cache reuse on CPUs~\cite{Nguyen:2010:BOS:1884643.1884658,Zhou:2012,Yount:2016:YSK:3019129.3019133,Unat2016,Devito}. As supercomputer architectures have evolved, the growing gap between DRAM bandwidth and FLOPs has focused research on data locality for bandwidth-bound computations.
    
    Several efforts have introduced MG frameworks targeting GPU architectures, including multiphysics applications with multigrid solvers on structured and unstructured grids~\cite{Amrex,Anderson,Grete,mali}. Despite progress in GPU acceleration and performance portability, detailed performance analysis and optimization for increasing GPU parallelism remain ongoing challenges.
    
    A growing body of work has demonstrated wafer-scale systems for scientific computing, including near-ideal weak scaling for stencil computations~\cite{scalable_25pt_stencil,Rocki,mathias,Sai,sai2}, matrix-free finite-volume solvers~\cite{sai_fv_arxiv,Ltaief}, molecular dynamics simulations~\cite{perez_md_sc24,perez-journal}, Monte Carlo particle transport kernels~\cite{Tramm24,Yoshii26}, and near-optimal collective implementations~\cite{luczynski_reduce}. Programmability efforts include DSL-based stencil approaches~\cite{brown}, automated stencil code generation~\cite{sai_codegen_sc24}, MLIR-based lowering pipelines~\cite{stawinoga_mlir_stencils_wse}, and the SPADA language for spatial architectures~\cite{spada}, alongside analytical modeling work~\cite{gianinazzi_spatialcomputer,gianinazzi2025energy,baumann2024low,luczynski_reduce}. However, these works primarily target single-level stencil operators or application-specific kernels.
    
    To the best of our knowledge, no prior work has demonstrated a complete geometric multigrid solver on a wafer-scale architecture. This work is the first end-to-end exploration of mapping a full GMG algorithm onto the Cerebras WSE-3, bridging the gap between stencil-centric studies and full multi-level scientific solvers.

    \section{Conclusion}
        We presented a mapping of GMG to WSE-3, achieving up to a \(25\times\) speedup over a single node NVIDIA H200 GPU.  As this is, to our knowledge, the first mapping of GMG to WSE-3, this work exposes the challenges of achieving efficiency for multi-scale algorithms, especially when memory size is so constrained. As a result of this work, we make a number of observations regarding mapping multigrid algorithms to tiled architectures.  First, the compute time dominates for the fine grids, but communication time dominates and increases execution time for coarser grids due to the strided communication.  Therefore, shallow V-cycles versus deep V-cycles or W-cycles are better suited for this architecture. The communication pattern required to skip over inactive PEs was optimized with a forwarding optimization that led to significant speedups for coarse grids.
        In spite of these challenges, deterministic, high-bandwidth memory access times and local network bandwidth lead to significant speedups over a single GPU, an iso-capability win.
