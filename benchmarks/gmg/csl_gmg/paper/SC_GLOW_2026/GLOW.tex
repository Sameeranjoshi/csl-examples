...
\begin{figure}[t]
                ...
                \begin{figure}[t]
                    ...
                    \caption{Top: Stencil Operator on WSE-3 2D grid for a \(8^3\) problem with 3 levels. The Highlighted nodes are the Active PEs and remaining Inactive PEs. The Hop distance differs as one keeps going down the levels and exchange halo regions H hops apart. }
...
The pre-existing modules assume 2-dimensional nearest-neighbor communication, which we extended to support multi-level strided communication, Dirichlet boundary conditions, and the forwarding optimization  (Section~\ref{sec:forwarding}). %Table~\ref{tab:code_breakdown} summarizes the implementation.} %along with layout module to map these operators on wafer PEs.
            ...
            \paragraph{Layout and coloring}
                The layout description partitions 9 colors and 7 task IDs across the three modules. The stencil library uses eight colors
                ...
                conflicts. The allreduce uses one additional color. %(C8) with dynamic teardown to reuse it across 4 pipeline stages. 
            ...
            \paragraph{Routing and multi-level adaptation}
                ...
                For restriction, we modified 
                %added \texttt{setWindowedReductionParams} to the 
                allreduce so that only a $2^l \times 2^l$ window of active PEs participates in each reduction. The forwarding optimization (Section~\ref{sec:forwarding}) further configures inactive PEs to relay wavelets directly through router queues, bypassing the memory ramp.

            \paragraph{State machine and callbacks}
                The V-cycle kernel (1,103 lines) executes entirely on-device as a 28-state fine grained state machine (Table~\ref{tab:state_machine}). The down-cycle advances through states that set up each level, apply pre-smoothing via the strided stencil, 
                %(\texttt{SMOOTH\_APPLY} $\to$ \texttt{SMOOTH\_UPDATE}, 
                looping for the configured number of iterations. compute the residual, %(\texttt{RESIDUAL\_APPLY} $\to$ \texttt{RESIDUAL\_COMPUTE})
                and restrict to the next coarser level. %(\texttt{RESTRICT\_REDUCE} $\to$ \texttt{RESTRICT\_DIV}). 
                At the coarsest $2\times2$ grid, a Jacobi bottom solver iterates. The up-cycle reverses: for each level it expands the z-dimension,
                %(\texttt{EXPAND\_Z}), 
                ...
                completes, the library invokes \texttt{f\_callback}, which advances the state variable and reactivates the state machine task, facilitating an implicit barrier the programming model. This design executes an entire V-cycle without returning control to the host; the host intervenes only between complete V-cycle iterations to read the convergence residual $\rho$.
...
\end{table*}
        ...
        \subsection{V-Cycle Variation Performance Comparison}
            ...
            Figure~\ref{fig:hpgmg-speedup} compares single V-cycle speedup of GLOW on WSE-3 against HPGMG on NVIDIA GH200 across five multigrid configurations. GLOW consistently outperforms the GH200 at every problem size, with speedups increasing as the grid scales. The best single V-cycle speedup of approximately $25\times$ is observed at the $512{\times}512$ grid for the $6/6/6$ (Shallow) configuration, while configurations with heavy coarse-level smoothing such as $4/4/100$ yield the lowest speedups.
        ...
        \subsection{Per-Operator Performance Breakdown}
            ...
            {\textbf{Smooth, Residual}
            %old: separate paragraphs for smooth (3 paras), residual (1 sentence), restriction (1 para)
            referring to Figure~\ref{fig:per-operation-timing} 
            above operators use the 7-point strided stencil (stride $2^l$ at level $l$) and share similar performance trend. Cost initially decreases toward coarser levels as active PEs and local data halve per level. Unlike conventional weak scaling where cost plateaus or improves monotonically, in GLOW we observe a gradual increase beyond a certain level due to growing hop distance.

            Figure~\ref{fig:spmv-internal} benchmarks the internal costs in detail: Compute time calculates the Laplacian across the x-y and z plane, it halves per level before flattening from fixed overheads. Communication measures the time to send and recv halo regions in all 4 directions, it starts below compute on fine grids but grows as halo messages traverse longer paths. The crossover point reflects the tradeoff between shrinking data volume and increasing distance. Table~\ref{tab:model_communication_costs} confirms this analytically for both strided and forwarding-optimized variants. Residual behaves similar except the differences in absolute time come from the absence of multiple smoothing iterations.

            \textbf{Restriction} uses a window-based reduction (Figure~\ref{fig:Spmv_and_restriction}) that gathers contributions from $N{-}1$ PEs; its cost grows with window size due to producer-consumer dependences and ramp cost. We apply forwarding optimization to keep this cost low.
            }

            \begin{figure*}[t]
                \centering
                \includegraphics[width=\textwidth]{figures/spmv_internal.png}
                \caption{Stencil micro-benchmark on CS-3: compute vs.\ communication time per level for the 7-point strided stencil used in smooth and residual operators.\textcolor{red}{I think this is the only place in the paper you use "SpMV".  You need to relabel this -- "Total Time"}}
...
\begin{tabular}{|l||r|r|r|r||r|r|r|r|} \hline
            ...
            \multicolumn{3}{|l||}{\textbf{Mem AI}: 0.1093} & \multicolumn{2}{l||}{\textbf{Fab AI}: 2.2232} & \multicolumn{2}{l|}{\textbf{Mem AI}: 0.1094} & \multicolumn{2}{l|}{\textbf{Fab AI}: 2.2232} \\ \hline
        \end{tabular}
    \end{minipage}
\end{tabular}
\end{table*}
        ...
        %compute bound
        %discussion on 
            Figure~\ref{fig:roofline} presents a roofline analysis for a $512^3$ problem with 9 levels using 262,144 active PEs. The left panel shows the per-PE (0,0) performance: finer levels (L0, L1) reach 43\% and 18\% of the 1.75 GFLOP/s per-PE peak respectively, operating in the compute-bound region. As levels coarsen, performance drops and shifts toward the memory/fabric-bound region due to growing communication distance and shrinking local computation. The right panel shows the active-PE system view, where the per-level peak scales with the number of active PEs at each level. L0 achieves 43\% of its 458.8 TFLOP/s system peak. The roofline confirms that GLOW is compute-bound at fine levels and transitions to communication-bound at coarse levels, consistent with our per-operator analysis. 
            \textcolor{red}{TODO: Write methodology, and talk about the table<todo add me>}
            ...
            Table~\ref{tab:roofline_ops} breaks down per-PE operations at the finest (L0) and a coarse level (L7). FMAC (multiply-accumulate from the stencil) dominates the FLOPs, while FMOV32 is the only operation that touches the fabric (halo exchange). Arithmetic intensity is constant across levels (0.109 FLOP/Byte for memory, 2.223 FLOP/Byte for fabric), but absolute work shrinks by $128\times$ from L0 to L7. Coarse levels have too little computation to amortize fixed communication overheads (router latency, growing hop distance), explaining the transition from compute-bound to communication-bound in Figure~\ref{fig:roofline}.
...
several competing concerns: memory footprint per PE, communication distance between levels, packing/repacking cost, stencil locality, chip utilization, and routing/coloring complexity. No single layout strategy dominates across all of these dimensions. The strided distributed mapping we chose prioritizes stencil parallelism and regular communication patterns; it avoids bulk inter-level data movement with irregular communication patterns.  This regularity came at increased communication costs at coarser levels and under-utilization of the wafer.  

There are several ways to increase wafer utilization that have the potential to either improve performance or increase the problem size supported. Since our chief bottleneck was memory, we might benefit from a 
...
To further reduce the forwarding cost in the strided communication, techniques to change the routing might be useful; e.g., using control wavelets and runtime coloring reconfiguration~\cite{mathias}.
