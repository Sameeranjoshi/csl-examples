...
\end{comment}
        ...
        \subsection{Per-Operator Performance Breakdown}
            ...
            \textbf{Restriction} uses a window-based reduction (Figure~\ref{fig:Spmv_and_restriction}) that gathers contributions from $N{-}1$ PEs; its cost grows with window size due to producer-consumer dependences and high ramp cost, we apply forwarding optimization to keep this cost low.
            }
            ...
            \textbf{Coarse solver.} The coarse solver applies weighted Jacobi relaxation on the coarsest grid. Its cost is very high compared to fine-level operators and grows with the number of coarse iterations, significantly impacting overall performance in deep V-cycles and W-cycles (Figure~\ref{fig:hpgmg-speedup}).
        ...
        \subsection{Memory Constraints: Code vs.\ Data Tradeoff}
            ...
            \begin{table*}[t]
                \caption{Per-PE operation breakdown for a $512^3$ V-cycle. FMAC counts 2 FLOPs; FMOV variants contribute 0. Mem(B)${}=(loads{+}stores){\times}4$. Fab(B): fabric traffic from halo exchange; only FMOV32 touches the fabric. Mem AI = Total FLOPs / Mem(B); Fab AI = Total FLOPs / Fab(B). Arithmetic intensity is identical across levels.}
    ...
    \section{Lessons Learned and Next Steps}
        ...
        such is demonstrated by Mathias et al.~\cite{mathias} using control wavelets and runtime coloring reconfiguration can be useful for reducing the curves we see in our experiments.
