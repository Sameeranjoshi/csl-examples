...
\end{table*}
        ...
        \subsection{Per-Operator Performance Breakdown}
            \begin{figure}[t]
                ...
                \caption{Per-operator time per level for one V-cycle on a \(512^3\) problem (GLOW on CS-3). Down and up phases are summed at each level. Config: \(6/6/6\) pre/post/coarse, 9 levels, FP32, 7-point stencil. breaks down time per operator for one V-cycle (excluding convergence), summed over down and up phases at each level
                }
