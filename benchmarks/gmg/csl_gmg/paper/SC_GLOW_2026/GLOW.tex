...
\end{comment}
        ...
        \subsection{Memory Constraints: Code vs. Data Tradeoff}
            ...
            {\color{blue}
            Table~\ref{tab:memory_utilization} quantifies this tradeoff across all problem sizes. Code size remains fixed at ${\sim}$23KB (47\%) regardless of problem size. Data grows from 2.9\% at $4^3$ to 50.7\% at $512^3$, where the PE is 98\% full. Wafer utilization ranges from 0.002\% ($4^3$, 16 PEs) to 29.4\% ($512^3$, 262K PEs), showing that even the largest problem we can fit uses less than a third of the wafer.
    ...
    \section{Lessons Learned and Next Steps}
        ...
        To quantify this, Table~\ref{tab:vcycle_vs_wcycle} compares V-cycle and W-cycle on a $256^3$ problem with 8 levels and 6/6/6 smoothing. Both converge in 10 iterations, but the W-cycle's average cycle time is $31.3\times$ slower because it visits level $L$ exactly $2^L$ times ($\gamma=2$), repeatedly paying the coarse-level communication penalty. The time to solution is $27.8\times$ worse despite a marginally better convergence rate (1.32 vs.\ 1.24 decades/iteration). This confirms that deeper cycle schedules are poorly matched to the strided mapping on WSE.

            \begin{table}[t]
                \caption{V-cycle vs.\ W-cycle on $256^3$, 8 levels, 6/6/6 smoothing.}
