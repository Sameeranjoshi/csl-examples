...
\keywords{Spatial dataflow architecture, Geometric multigrid, Wafer-Scale Engine}
...
\textcolor{blue}{
  ...
  ▎ The implementation of GMG on WSE-3 requires composing all of the programming model elements described in Section 2.2.3—layout, coloring, routing, tasks, DSDs, and runtime router reconfiguration—into a single coordinated
  ...
  ▎ Layout and coloring. The layout module (layout_gmg_vcycle.csl, 157 lines) defines a 2D PE grid on the 762×1172 WSE-3 fabric. It allocates 9 colors for inter-PE communication—8 for the stencil operator's directional halo
  ...
  ▎ Routing and forwarding. Static routing is established at compile time: each PE's comptime block configures 16 @set_local_color_config directives that wire input/output queues to the four cardinal directions via the
  ...
  ▎ Kernel and state machine. The V-cycle kernel (kernel_gmg_vcycle.csl, 1,103 lines) implements the full multigrid algorithm as an on-device state machine with 28 states. Each GMG operator—smooth, residual, restriction,
  ...
  ▎ Data management within 48KB. Each PE stores the solution vector u, right-hand side f, residual r, and stencil output Au for the current level, plus multi-level save arrays (save_u_smooth, save_f_down) sized by the
