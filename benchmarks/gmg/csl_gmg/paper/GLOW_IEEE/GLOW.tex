...
Geometric multigrid (GMG) is an important family of algorithms used by computational scientists to accelerate the convergence of iterative solvers for linear systems of the form \(Ax=b\). It is a multigrid because computation is mapped to a hierarchy of coarser grid levels.  As shown in Figure~\ref{fig 1:V-cycle in GMG}, the underlying GMG operators are organized in a V-cycle that includes stencil application, residual, restriction, interpolation, smooth, and a direct (bottom) solver. %\textcolor{red}{OA: add here that is a V-cycle}. 
In such a formulation, the floating-point computation in GMG is dwarfed by the overhead of data movement (during stencil application and across levels), making managing the memory hierarchy and cross-processor communication critical to achieving high performance.    
...
This paper presents 
evaluates the implications of mapping GMG to 
...
In this paper, we present GLOW (Gmg Levels On Wafer).  We build on the lessons from successful use of Cerebras for stencil computations to provide a mapping of an algorithm that is
...
complexities, and amount of chip utilization and power consumption.  These are captured in Table~\ref{tab:layout_comparison}.
...
{\color{blue}
  ...
  \noindent\textbullet{} Layout and coloring. The layout module (layout_gmg_vcycle.csl, 157 lines) defines a 2D PE grid on the 762×1172 WSE-3 fabric. It allocates 9 colors for inter-PE communication—8 for the stencil operator's directional halo
  ...
  requirements. The layout then iterates over every PE, generating position-dependent parameters (boundary flags, per-PE stencil routing) and binding them to a single kernel file via @set_tile_code.

  \noindent\textbullet{} Routing and forwarding. Static routing is established at compile time: each PE's comptime block configures 16 @set_local_color_config directives that wire input/output queues to the four cardinal directions via the
  ...
  route reconfiguration. At each level transition, setHopBasedParams recomputes which PEs are active, which are interior forwarders, and adjusts the send/receive patterns accordingly. The forwarding optimization (Section
  ...
  \noindent\textbullet{} Kernel and state machine. The V-cycle kernel (kernel_gmg_vcycle.csl, 1,103 lines) implements the full multigrid algorithm as an on-device state machine with 28 states. Each GMG operator—smooth, residual, restriction,
  interpolation, coarse solve, and convergence check—maps to one or more states. Transitions are callback-driven: when an asynchronous stencil or allreduce completes, the library invokes f_callback, which advances the state
  variable and reactivates the state machine task via @activate. This design executes the entire V-cycle without returning control to the host between states; the host only intervenes between complete V-cycle iterations to
  read convergence data.

  \noindent\textbullet{} Data management within 48KB. Each PE stores the solution vector u, right-hand side f, residual r, and stencil output Au for the current level, plus multi-level save arrays (save_u_smooth, save_f_down) sized by the
  ...
  (element duplication). The stencil communication libraries add four directional buffers of BLOCK_SIZE each for halo exchange, bringing per-PE data pressure close to the 48KB limit for the largest problem sizes. At 512\(^3\)
