...
def main(layout, platform):
    ...
    ############################################################
    # Layout setting for the problem.
    ############################################################
    # From GMGOscar there are details on what is the problem size and how many levels are needed.
    # Accordingly query it and start laying out the code regions.
    # For example, if the problem size is 1024x1024 and the number of levels is 10, then the code regions should be laid out as follows:
    # level: problem size -> (startx, starty) (width, height)
    # Find pattern in above and start laying it out, the layers will have a kernel_gmg.csl file on them, each has a same kernel for now.
    # The layout should be done in the layout function, so we can just call it here.
    # place them as per below pattern:
    # M, N = 1024, 1024
    # 1. L0: 1024x1024 --> (0,0) (M, N)
    # 2. L1: 512x512 --> (0,N) (M/2, N/2)
    # 3. L2: 256x256 --> (0,N+N/2) (M/4, N/4)
    # 4. L3: 128x128 --> (0,N+N/2+N/4) (M/8, N/8)
    # 5. L4: 64x64 --> (0,N+N/2+N/4+N/8) (M/16, N/16)
    # 6. L5: 32x32 --> (0,N+N/2+N/4+N/8+N/16) (M/32, N/32)
    # 7. L6: 16x16 --> (0,N+N/2+N/4+N/8+N/16+M/32) (M/64, N/64)
    # 8. L7: 8x8 --> (0,N+N/2+N/4+N/8+N/16+M/32+M/64) (M/128, N/128)
    # 9. L8: 4x4 --> (0,N+N/2+N/4+N/8+N/16+M/32+M/64+M/128) (M/256, N/256)
    # 10. L9: 2x2 --> (0,N+N/2+N/4+N/8+N/16+M/32+M/64+M/128+M/256) (M/512, N/512)

    code_regions = []
    start_x = 0
    start_y = 0
    # Dynamically create code regions for each GMG level using device_solver.grids
    for level in range(args.levels):
        level_width, level_height = device_solver.grids[level]['nx'], device_solver.grids[level]['ny']
        layer = layout.create_code_region("./dummy.csl", f"l{level}", level_width, level_height)
        layer.place(start_x, start_y)
        start_y += level_height
        start_x = 0
        code_regions.append(layer) 
    ...
    simulator = SdkRuntime(artifacts, platform, memcpy_required=False)
...
def layout():
    ...
    platform = get_platform(None, SimfabConfig(dump_core=True), target)
    layout = SdkLayout(platform)
    return layout, platform
    
if __name__ == "__main__":
    layout, platform = layout()
    main(layout, platform)
