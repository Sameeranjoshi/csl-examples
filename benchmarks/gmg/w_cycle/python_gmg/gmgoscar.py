...
class SimpleGMG:
    ...
    def v_cycle(self, level: int = 0):
        """W-cycle multigrid (gamma=2): standard recursive γ-cycle that calls
        MGC(level+1) twice per MGC(level) invocation. Kept under the name
        `v_cycle` so existing callers (solve/solve_iterative) pick it up
        unchanged; this folder is the W-cycle variant."""
        GAMMA = 2
        ...
        # Initialize coarse solution (zeroed ONCE before the γ-fold recursion)
        self.grids[level + 1]['u'].fill(0.0)

        # γ recursive calls to coarser level (W-cycle when γ=2).
        # u_{level+1} accumulates across calls per standard γ-cycle definition.
    ...
    def solve_iterative(self, max_iter: int = 20):
        ...
        while (xi_max > self.rel_tolerance) and (iterations < max_iter):
            # Perform W-cycle (γ=2): recursive γ-cycle from level 0
            cycle_start = time.time()
            self.v_cycle(0)  # v_cycle in this folder is W-cycle (see its docstring)
            cycle_time = time.time() - cycle_start
