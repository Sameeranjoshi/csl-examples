"""
Compile and run GMG W-cycle for WSE3 (γ=2 multigrid).
This file is the W-cycle variant of csl_gmg/compile_and_run_wse3.py: the kernel
state machine in src/kernel_gmg_vcycle.csl now calls MGC(L+1) γ=2 times per
MGC(L), so coarsest level is visited 2^(levels-1) times per outer iteration.
...
def get_out_path(size, levels, max_ite, pre_iter, post_iter, bottom_iter, suffix=""):
    ...
    Prefix 'w_' distinguishes W-cycle artifacts from the V-cycle out_dir_* in csl_gmg/."""
    return f"w_out_dir_S{size}x_L{levels}_M{max_ite}_P{pre_iter}_P{post_iter}_B{bottom_iter}{suffix}"
...
def run_check_memory_for_outputs(script_dir=None, out_paths=None):
    ...
    try:
        if out_paths is not None:
            ...
            out_dirs = sorted(glob.glob("w_out_dir_*"))
            if not out_dirs:
                print("No w_out_dir_* folders found for memory check")
                return

        for out_path in out_dirs:
            if not os.path.isdir(out_path):
                continue
            # Parse size from out_path: w_out_dir_S4x_L2_... -> 4
            size_match = re.search(r"w_out_dir_S(\d+)x_", out_path)
            size = int(size_match.group(1)) if size_match else None
            artifact_name = f"out_wcycle{size}" if size is not None else "out_wcycle"
...
def process_on_device(size, levels, channels, max_ite, abs_tolerance, pre_iter, post_iter, bottom_iter):
    ...
    artifact_name = f"out_wcycle{size}"
...
def main():
    ...
    #
    # W-cycle (γ=2): level l is visited 2^l times per outer iteration, so the
    # coarsest level is visited 2^(levels-1) times. For levels=9 that's 256 bottom
    # solves per iteration — keep the active list small. The active problems below
    # target one or two sizes. Uncomment additional entries as needed.

    problems = [
        # === Active W-cycle test problems (keep this list small) ===
        (256, 8, 100, 1e-2, 6, 6, 6),   # Very large — fits after R1+R2 optimizations 
        # === End active list ===
    ]
    unused_problems = [
        # # 6/6/100
